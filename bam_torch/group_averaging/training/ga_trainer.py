...
from bam_torch.group_averaging.model import ACTIVE_FN_REGISTRY, MODEL_REGISTRY
...
class GATrainer(BaseTrainer):
    ...
    def __init__(self, json_data, rank, world_size):
        ...
        (
            self.transform,
            self.model_forward_cls,
            self.ga_method,
            self.group_averaging,
            self.permute,
    ...
    def train_one_epoch(self, mode="train", data_loader=None):
        ...
        for i, data in enumerate(data_loader):
            ...
            batch, entropy_loss = self.transform(
                ...
                n_samples=self.json_data.get(
                    "nsamples"
                ),  # for the probabilistic symmetrization
            )
    ...
    def set_model(self):
        ...
        if model_name in ["faenet", "gnn"]:
            ...
            force_decoder_model_config = {
                "simple": {"hidden_channels": 128, "norm": "batch1d"},
                "mlp": {"hidden_channels": 256, "norm": "batch1d"},
                "res": {"hidden_channels": 128, "norm": "batch1d"},
                "res_updown": {"hidden_channels": 128, "norm": "batch1d"},
            }
            model = model_cls(
        ...
        if model_config.get("ga_method").lower() in [
            "prob",
            "probabilistic",
            "prob_rot",
            ...
            small_equiv_model_config_params = {
                ...
                "output_irreps": small_equiv_model_config.get(
                    "output_channels", "3x1o"
                ),
                "gate": gate_name,
            ...
            interface_n_params = sum(
                p.numel() for p in self.equiv_model.parameters() if p.requires_grad
            )
            print(
