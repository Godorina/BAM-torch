...
class RACECalculator(Calculator, MultiheadEvaluator):
    ...
    def __init__(
        self, json_data=None, model=None, device=None, element_wise=True, multihead=None
    ):
        """
        ...
        if model is not None and json_data is None:
            model_ckpt = torch.load(model, map_location=self.device, weights_only=False)
    ...
    def configure_head(self, multihead):
        ...
        if multihead_config == {}:
            ...
            self.json_data["NN"]["foundation_model"] = self.json_data["NN"]["fname_pkl"]
