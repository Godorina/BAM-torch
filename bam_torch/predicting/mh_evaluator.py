...
from bam_torch.utils.utils import date, find_input_json
...
class MultiheadEvaluator(Evaluator):
    ...
    def configure_model(self):
        ...
        if datasets and len(datasets) > 0:
            ...
            # Single-head model
            heads = None
            print("✅ Loading RACEUnified (single-head mode)")
        ...
        hidden_irreps = o3.Irreps(
            model_config.get("hidden_channels", self.json_data["hidden_channels"])
        )
        ...
        num_basis_func = model_config.get(
            "num_radial_basis", self.json_data["num_radial_basis"]
        )
        max_ell = model_config.get("max_ell", self.json_data["max_ell"])
        avg_num_neighbors = model_config.get(
            "avg_num_neighbors", self.json_data["avg_num_neighbors"]
        )
        ...
        regress_forces = model_config.get(
            "regress_forces", self.json_data.get("regress_forces")
        )
    ...
    def evaluate(self, element_wise=True):
        ...
        if is_multihead:
            ...
            if isinstance(eval_head, str):
                eval_head = (
                    head_names.index(eval_head) if eval_head in head_names else 0
                )
            ...
            if eval_head in per_head_enr_avg:
                head_e0s = per_head_enr_avg[eval_head].get(
                ...
                print("   ⚠️ No per-head E0s found, using global E0s")
        else:
        ...
        if is_multihead:
            ...
            if eval_head in per_head_scale_shift and per_head_scale_shift[eval_head]:
                ...
                element_wise = False
                print("   ⚠️ No per-head scale_shift found, using 0")
        else:
        ...
        for i, data in enumerate(self.data_loader):
            ...
            if is_multihead:
                ...
                data["head"] = torch.full(
                    (num_graphs,), eval_head, dtype=torch.long, device=self.device
                )
            ...
            # Correct the energy
            if element_wise:
                e_corr = torch.tensor([e_corr_[int(iz)] for iz in species]).sum()
            else:
                e_corr = e_corr_
            ...
            mae_e = (
                float(loss_dict.get("loss_e", 0.0).detach().cpu())
                if isinstance(loss_dict.get("loss_e"), torch.Tensor)
                else 0.0
            )
            mae_f = (
                float(loss_dict.get("loss_f", 0.0).detach().cpu())
                if isinstance(loss_dict.get("loss_f"), torch.Tensor)
                else 0.0
            )

            print(
                f"{date()}  {i:6d} | {pred_energy:<15.6f} {mae_e:<15.6g} {mae_f:<15.6g} | {exact_energy:<15.6f} |"
            )
            print(
                f"{date()}  {i:6d} | {pred_energy:<15.6f} {mae_e:<15.6g} {mae_f:<15.6g} | {exact_energy:<15.6f} |",
                file=self.fout,
