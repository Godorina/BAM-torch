...
class Evaluator(BaseTrainer):
    ...
    def evaluate(self, element_wise=True):
        ...
        for i, data in enumerate(self.data_loader):
            ...
            mae_values["energy"].append(
                torch.abs(preds["energy"].flatten() - data["energy"].flatten())
                .detach()
                .cpu()
            )
            if "forces" in preds:
                mae_values["forces"].append(
                    torch.abs(preds["forces"].flatten() - data["forces"].flatten())
                    .detach()
                    .cpu()
                )
            if "stress" in preds:
                try:
                    mae_values["stress"].append(
                        torch.abs(preds["stress"].flatten() - data["stress"].flatten())
                        .detach()
                        .cpu()
                    )
                except Exception:
    ...
    def _run_optional_catalyst_economics(self, model_metrics):
        ...
        combined_report = Path(
            economics_config.get(
                "combined_report", out_dir / "bam_catalyst_economics_report.json"
            )
        )
        merge_model_and_economics_report(
        ...
        print(
            f"[ECONOMICS] Combined BAM + catalyst report -> {combined_report}",
            file=self.fout,
        )
    ...
    def get_scale_shift_correction(self, element_wise):
        if element_wise:
            try:
                e_corr = torch.tensor(self.model_ckpt["valid_scale_shift"]).mean()
                element_wise = False
            except:
    ...
    def configure_dataloader(self):
        ...
        data_loader, uniq_element, enr_avg_per_element = get_dataloader_to_predict(
    ...
    def configure_logger(self):
        ...
        atexit.register(
            lambda: on_exit(fout, separator, self.n_params, self.json_data, self.date1)
        )
