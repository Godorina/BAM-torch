...
def get_enr_avg_per_element(traj, element):
    ...
    element_counts = {
        i: np.array([(atoms.numbers == e).sum() for atoms in traj])
...
def get_enr_avg_per_element_with_ztable(
...
def get_graphset(
    ...
    for atoms in iterator:
        ...
        node_enr_avg = np.array(
            [enr_avg_per_element[uniq_element[iz]] for iz in atoms.numbers]
        )
        ...
        graph = Data(
            ...
            volume=torch.tensor(
                [volume] if np.isscalar(volume) else volume, dtype=torch.float32
            ),
        )
...
def get_graphset_with_pad(graphset, pad_nodes_to, pad_edges_to):
    ...
    for data in graphset:
        ...
        if n_nodes < pad_nodes_to:
            ...
            data.species = torch.cat([data.species, padding[:, 0]], dim=0).to(
                torch.long
            )
            data.num_nodes = pad_nodes_to
            node_mask = torch.cat(
                [
                    torch.ones(original_n_nodes),
        ...
        if n_edges < pad_edges_to:
            ...
            edge_index_padding = torch.zeros(
                (2, pad_edges_to - n_edges), dtype=torch.long
            )
...
def get_dataloader(
    ...
    if element == None or element == "auto":
        element = sorted(
            list(set(atom.number for atoms in traj for atom in atoms))
        )  # traj: ase.Atoms
...
def get_dataloader_multihead(
    datasets_config,
    cutoff,
    nbatch,
    regress_forces=True,
    max_neigh=None,
    foundation_enr_avg=None,
    foundation_uniq_element=None,
    rank=0,
    world_size=1,
    smoke_config=None,
):
    ...
    for head_idx, ds_config in enumerate(datasets_config):
        ...
        if smoke_enabled:
            ...
            if rank == 0:
                print(
                    f"  - [SMOKE] Limited to {len(train_data)} train, {len(valid_data)} valid"
                )
        ...
        element = sorted(
            [int(z) for z in set(atom.number for atoms in traj for atom in atoms)]
        )
        ...
        if foundation_uniq_element is not None:
            ...
            if use_foundation_e0s and foundation_enr_avg is not None:
                ...
                enr_var = 1.0
                if rank == 0:
                    print("  - Using foundation E0s (foundation z_table)")
            else:
                ...
                if rank == 0:
                    print("  - Calculated E0s for this head (foundation z_table)")
        else:
            # No foundation -> compute E0s using original method
            enr_avg_per_element, uniq_element, enr_var = get_enr_avg_per_element(
                traj, element
            )
            if rank == 0:
                print("  - Calculated new enr_avg_per_element (no foundation)")
        ...
        for data, graphs_list, data_type in [
            (train_data, all_train_graphs, "train"),
    ...
    for h in sorted(
        set(list(per_head_train_graphs.keys()) + list(per_head_valid_graphs.keys()))
    ):
        # Train loader per head
        train_gs = per_head_train_graphs.get(h, [])
        random.shuffle(train_gs)
        sampler_t = (
            DistributedSampler(train_gs, num_replicas=world_size, rank=rank)
            if world_size > 1
            else None
        )
        ...
        sampler_v = (
            DistributedSampler(valid_gs, num_replicas=world_size, rank=rank)
            if world_size > 1
            else None
        )
    ...
    if world_size > 1:
        data_sampler_train = DistributedSampler(
            all_train_graphs, num_replicas=world_size, rank=rank
        )
        data_sampler_valid = DistributedSampler(
            all_valid_graphs, num_replicas=world_size, rank=rank
        )

    train_loader = DataLoader(
        all_train_graphs,
        nbatch,
        shuffle=(data_sampler_train is None),
        drop_last=False,
        pin_memory=True,
    ...
    valid_loader = DataLoader(
        ...
        drop_last=False,
        pin_memory=True,
        sampler=data_sampler_valid,
    )

    if rank == 0:
        print("\n✓ Multihead Dataloaders created")
        print(
            f"  - Total train: {len(all_train_graphs)}, valid: {len(all_valid_graphs)}"
        )
    ...
    return (
        ...
        per_head_valid_loaders,
        global_uniq_element,
        global_enr_avg_per_element,
        per_head_enr_avg,
    )


def get_graphset_to_predict(
    data, cutoff, uniq_element, regress_forces=True, max_neigh=None
):
    graph_list = []
    for atoms in data:
        if atoms.calc:
        ...
        _, neighbors = torch.unique(
            torch.tensor(iatoms, dtype=torch.long), return_counts=True
        )
        species = np.array([uniq_element[iz] for iz in atoms.numbers])
        num_nodes = crds.shape[0]
        num_edges = iatoms.shape[0]
...
def get_dataloader_to_predict(
    fname, ndata, nbatch, cutoff, model_ckpt, regress_forces=True, max_neigh=None
):
    if type(ndata) == str:
    ...
    graphset = get_graphset_to_predict(
        traj, cutoff, uniq_element, regress_forces, max_neigh
    )
...
def data_to_dict(data):
    ...
    data_dict = {
        k: (torch.tensor(v) if isinstance(v, int) else v) for k, v in data_dict.items()
    }
    data_dict = {
        k: (torch.tensor(v) if isinstance(v, list) else v) for k, v in data_dict.items()
    }
    return data_dict
