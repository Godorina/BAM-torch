...
def _resolve_race_interaction_block(name: str):
    try:
        ...
        raise ValueError(f"Unknown interaction_block {name!r}. Valid options: {valid}")
...
from bam_torch.utils.scatter import scatter_mean, scatter_sum
...
class RACE(torch.nn.Module):
    ...
    def forward(
        ...
        node_f_logvar_ts = torch.zeros(
            (node_feats.shape[0], 6), device=node_energy.device
        )
        # Uncertainty quantification
        if str(self.output_irreps) == "8x0e":
            node_logvar_ts = torch.stack(
                node_logvar, dim=-1
            )  # [nbatch*num_nodes, nlayers]
            node_logvar_ts = node_logvar_ts.mean(dim=-1)  # [nbatch*num_nodes]
            # force variance L Voigt notation - xx, yy, zz, yz, xz, xy
            node_f_logvar_ts = torch.stack(
                node_f_logvar, dim=-1
            )  # [nbatch*num_nodes, 6, nlayers]
        ...
        node_frc_var = torch.cat(
            [torch.exp(node_f_logvar_ts[:, :3]), node_f_logvar_ts[:, 3:]], dim=1
        ).view(-1, 6)
        ...
        if "auto" in self.regress_forces:
            ...
            stress = torch.sum(
                node_stress, dim=-1
            )  # [nbatch*num_nodes, 6]  # total_energy
            stress = scatter_sum(
                src=stress,
                index=data["batch"],
                dim=0,
...
class RACEUnified(torch.nn.Module):
    ...
    def __init__(
        ...
        regress_forces: str = "direct",
        compute_stress: bool = True,
        heads: list[str] | None = None,  # ⭐ Multihead support
        l_separated_layer_norm: bool = False,
        interaction_block: str = "slow",
        radial_polynomial_p: int = 2,
    ):
...
class MACE(torch.nn.Module):
    ...
    def __init__(
        ...
        output_irreps: e3nn.o3.Irreps = o3.Irreps("1x0e"),
        active_fn: str = "swish",
        radial_MLP: list[int] | None = [64, 64, 64],
        ...
        atomic_inter_shift = [0.0] * len(
            heads
        )  # determine_atomic_inter_shift(args.mean, heads)
