...
class FromChunks(torch.nn.Module):
    ...
    def forward(
        self, chunks: list[torch.Tensor], leading_shape: tuple[int]
    ) -> torch.Tensor:
        if len(self.irreps_dims) != len(chunks):
            raise ValueError(
                f"from_chunks_torch: len(irreps) != len(chunks), {len(self.irreps_dims)} != {len(chunks)}"
            )

        for x in chunks:
            if x is not None and not isinstance(x, torch.Tensor):
                raise ValueError(
                    "from_chunks_torch: chunks contains non-tensor elements"
                )
        ...
        if dtype is None:
            raise ValueError(
                "from_chunks_torch: Need to specify dtype if chunks is empty or contains only None."
            )
        ...
        if sum(mul * dim for mul, dim in self.irreps_dims) > 0:
            array = torch.cat(
                [
                    (
                        torch.zeros(
                            leading_shape + (mul * dim,),
                            dtype=dtype,
                            device=chunks[0].device,
                        )
                        if x is None
                        else x.reshape(leading_shape + (mul * dim,))
                    )
            ...
            array = torch.zeros(
                leading_shape + (0,), dtype=dtype, device=chunks[0].device
            )
...
class TensorRegroupByIrreps(torch.nn.Module):
    ...
    def forward(
        ...
        array = torch.cat(
            [
                torch.zeros(leading_shape + (mul_ir_dim,), dtype=dtype)
                if x is None
...
class ConcatenateIrrepsTensor(torch.nn.Module):
    ...
    def forward(self, tensors: list[torch.Tensor], axis: int = -1) -> torch.Tensor:
        if axis < 0:
            axis += tensors[0].ndim
...
class TensorIrrepsArrayProduct(torch.nn.Module):
    ...
    def forward(self, tensor: torch.Tensor, irreps_array: torch.Tensor) -> torch.Tensor:
        x1 = irreps_array
        x1_chunks = split_tensor_by_irreps(x1, self.irreps_dims)
        x2 = tensor
        offsets = torch.cumsum(
            torch.tensor([0] + [mul for mul, _ in self.irreps_dims]), dim=0
        )
        chunks = [
