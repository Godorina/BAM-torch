...
from bam_torch.utils.utils import date, get_dataloader, on_exit
...
class BaseTrainer:
    ...
    def train(self):
        ...
        for epoch in range(nepoch):
            ...
            # Save check point
            if (epoch + 1) % self.json_data["NN"][
                "nsave"
            ] == 0 and not self.l_ckpt_saved:
    ...
    def print_logger(self, epoch, epoch_loss_train, epoch_loss_valid):
        ...
        self.logger.print_epoch_loss(step_dict, epoch_loss_train, epoch_loss_valid, lr)

    def configure_dataloader(self):
        train_loader, valid_loader, uniq_element, enr_avg_per_element = get_dataloader(
    ...
    def configure_logger(self):
        ...
        if self.rank == 0:
            ...
            atexit.register(
                lambda: on_exit(
                    fout, separator, self.n_params, self.json_data, self.date1
                )
            )
    ...
    def get_unique_filename(self):
        ...
        while os.path.exists(
            fname
        ):  # if exist the file of ```fname``` in this directory
            old_name = fname
    ...
    def configure_model(self):
        ...
        if restart:
            ...
            model_ckpt = torch.load(
                model_config["fname_pkl"], map_location=self.device, weights_only=False
            )
        ...
        if evaluate:  # True or False(None)
            ...
            start_epoch = 0
            model_ckpt = torch.load(
                evaluate_config["model"], map_location=self.device, weights_only=False
            )
            model.load_state_dict(model_ckpt["params"])
            model.eval()
    ...
    def set_model(self):
        ...
        if (
            "radial_polynomial_p" in model_params
            and "radial_polynomial_p" in model_config
        ):
            model_kwargs["radial_polynomial_p"] = model_config["radial_polynomial_p"]
        ...
        if "interaction_block" in model_params:
            ...
            self.msg += (
                f"\ninteraction block:\n\033[33m -- {interaction_block}\033[0m\n"
            )
    ...
    def check_parameter_sync(self):
        for name, param in self.model.named_parameters():
            ...
            print(
                f"Rank {self.rank}, Parameter name: {name}, Shape: {param.shape}, Num.: {param.numel()}"
            )
